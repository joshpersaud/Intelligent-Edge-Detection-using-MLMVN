...
function [hidneur_weights, outneur_weights, iterations, RMSE] = Net_learn_rmse(X, y_d, hidneur_num, outneur_num, sec_nums, RMSE_thresh, local_thresh)
% This function utilizes the batch MLMVN-LLS learnng algorithm based on the
% RMSE convergence for multiple output neurons starting from random weights
...
while ( LearnFlag == 1)
    ...
    hid_outmat1 = [col_app hid_outmat];

    %Compute the output of the network
    z_outneur = hid_outmat1 * outneur_weights;
